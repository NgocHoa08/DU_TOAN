...
// Find the exact end of the last preset entry before '    };'
// Pattern: last '      }' followed by newline then '    };'
const marker = "      }\r\n    };";
const markerLF = "      }\n    };";

let idx = content.lastIndexOf(marker);
if (idx >= 0) {
  content = content.slice(0, idx) + newPresets + '\r\n    };' + content.slice(idx + marker.length);
  console.log('Inserted using CRLF marker');
} else {
  idx = content.lastIndexOf(markerLF);
  if (idx >= 0) {
    content = content.slice(0, idx) + newPresets + '\n    };' + content.slice(idx + markerLF.length);
...
// Verify
const count = (content.match(/stt\d+_/g) || []).length;
console.log('STT entries found:', count);
['stt1_', 'stt9_', 'stt15_', 'stt16_'].forEach(k => {
  console.log(k + ':', content.includes(k) ? 'OK' : 'MISSING');
});
