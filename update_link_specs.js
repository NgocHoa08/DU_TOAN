...
if (sIdxLink === -1 || eIdxLink === -1) {
  console.error('Could not find linkSpecsToDevices block, sIdx:', sIdxLink, 'eIdx:', eIdxLink);
  process.exit(1);
}
...
const newLinkSpecsCode = `function linkSpecsToDevices(allSpecs) {
      ...
      devs.forEach(function (d, dIdx) {
        ...
        if (lookupStt === 12 || (d.model && d.model.includes('XGS 128')) || d.name.toLowerCase().includes('tường lửa')) {
        ...
        if (lookupStt === 16 || d.name.toLowerCase().includes('cáp mạng') || d.model === 'CAT6') {
          d.name = 'Cáp mạng CAT 6 Việt Hàn CAT6';
          d.model = 'Việt Hàn CAT6';
          d.brand = 'Việt Hàn';
          d.origin = 'Việt Nam';
        } else if (lookupStt === 17 || d.name.toLowerCase().includes('cân bằng tải') || d.model === 'ER707-M2') {
